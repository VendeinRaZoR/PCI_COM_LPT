...
variable pin_planner_group pci_sample
variable pcicomp_dir "$env(QUARTUS_ROOTDIR)/../ip/altera/pci_compiler/lib//ip_toolbench"
variable SHARED_CLK no
variable SHARED_CLK_NAME null
variable INTERNAL_ARBITER no
variable PCI_SPEED 33
...
global no_device
if { $device == "AUTO" } {
   error "Error: \"AUTO\" device selected. Please select a specific device and re-source the PCI Constraints file"
}

# Sourcing the relevant files based on selected device 
switch $device_family {
   "Cyclone"             { source $pcicomp_dir/../../const_files/pci_cyclone_cf.tcl }
   "Cyclone II"          { source $pcicomp_dir/../../const_files/pci_cycloneii_cf.tcl }
   "Cyclone III"         { source $pcicomp_dir/../../const_files/pci_cycloneiii_cf.tcl }
   "Cyclone IV GX"   	 { source $pcicomp_dir/../../const_files/pci_cycloneiv_cf.tcl }
   "Cyclone IV E"   	 { source $pcicomp_dir/../../const_files/pci_cycloneive_cf.tcl }
   "Cyclone III LS"	 { source $pcicomp_dir/../../const_files/pci_cyclonelps_cf.tcl }
   "Stratix"             { source $pcicomp_dir/../../const_files/pci_stratix_cf.tcl }
   "Stratix GX"          { source $pcicomp_dir/../../const_files/pci_stratixgx_cf.tcl }
   "Stratix II"          { source $pcicomp_dir/../../const_files/pci_stratixii_cf.tcl }
   "Stratix II GX"       { source $pcicomp_dir/../../const_files/pci_stratixiigx_cf.tcl }
   ...
   "Stratix V"           { source $pcicomp_dir/../../const_files/pci_stratixv_cf.tcl }
   "MAX II"              { source $pcicomp_dir/../../const_files/pci_maxii_cf.tcl }
   ...
   "Arria GX"         	 { source $pcicomp_dir/../../const_files/pci_arriagx_cf.tcl }
   "Arria II GX"	 { source $pcicomp_dir/../../const_files/pci_arriaii_cf.tcl }
   "Arria II GZ"	 { source $pcicomp_dir/../../const_files/pci_arriaiigz_cf.tcl }		
   default {
      puts "device: $device -- device family = $device_family"
